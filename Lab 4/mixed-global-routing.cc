...
main(int argc, char* argv[])
{
    ...
    for (auto& flow : stats)
    {
        auto& stat = flow.second;
        NS_LOG_UNCOND("Flow ID: " << flow.first << " (" << stat.txPackets << " tx packets)");
        NS_LOG_UNCOND("Packet Delivery Ratio: " << stat.rxPackets * 100.0 / stat.txPackets << "%");
        NS_LOG_UNCOND("Average End-to-End Delay: " << stat.delaySum.GetSeconds() / stat.rxPackets
                                                   << "s");
        NS_LOG_UNCOND("Throughput: " << stat.rxBytes * 8.0 /
                                            (stat.timeLastRxPacket.GetSeconds() -
                                             stat.timeFirstTxPacket.GetSeconds()) /
                                            1000
                                     << " Kbps");
        NS_LOG_UNCOND("Overhead: " << (stat.txBytes - stat.rxBytes) * 8.0 / stat.txBytes * 100.0 << "%");
